...
export function YearProvider({ children }: { children: ReactNode }) {
  ...
  const preferredYear = useMemo(() => {
    ...
    // 2. Default to latest year from API or 2025
    if (years.length > 0) {
  ...
  // After hydration, prefer the locally stored year if available
  useEffect(() => {
    if (typeof window === 'undefined') {
      return;
    }
    ...
    const stored = window.localStorage.getItem('selectedYear');
    if (!stored) {
      return;
    }
    ...
    if (years.length > 0 && !years.some((entry) => entry.year === parsed)) {
      return;
    }
    setYear(parsed);
  }, [year, years, setYear]);
  ...
  const value = useMemo(() => ({
    ...
    currentYearSettings,
    isLoading,
  }), [year, setYear, years, currentYearSettings, isLoading]);
